...
avgexp <- AverageExpression(fov_stromal)
avgexp <- as.data.frame(avgexp$RNA)
avgexp$gene <- rownames(avgexp)
write.csv(avgexp, "avgexp_stromal.csv")
...
allmarkers <- read.csv("./Plots/7_Clustering/Option 2/Stromal_new/Harmony/Markers_stromal_harmony_clusters_RNA.csv")
...
fov_stromal$CIPR_hpca_clusters <- NULL
CIPR_hpca_clusters <- c("Fibroblast", "Epithelial cell", "Epithelial cell", "SPP1+MMP12+ Macrophage", "Endothelial cell",
                        ...
                        'Plasma cell') 
names(CIPR_hpca_clusters) <- levels(fov_stromal)
fov_stromal <- RenameIdents(fov_stromal, CIPR_hpca_clusters)
...
gut_ref_stromal <- read.csv("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/CIPR/gut_ref_stromal.csv")
colon_gut_ref_stromal <- read.csv("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/CIPR/colon_gut_ref_stromal.csv")
...
CIPR(input_dat = avgexp,
     comp_method = "all_genes_spearman", 
     reference = "custom",
     custom_reference = colon_gut_ref_stromal,  
     plot_ind = F,
     plot_top = T, 
     global_results_obj = T, 
     global_plot_obj = F)

write.csv(CIPR_top_results, "CIPR_top_results_colon_gut_ref_stromal.csv")

# 15.2.4- Add CIPR cluster lables
Idents(fov_stromal) <- "st_harmony_clusters"
levels(fov_stromal)

fov_stromal.markers <- FindAllMarkers(fov_stromal, only.pos = TRUE, logfc.threshold = 0.25) #min.pct = 0.25,
...
#fov_stromal$CIPR_gut_clusters <- NULL
#fov_stromal$CIPR_colon_gut_clusters <- NULL

CIPR_colon_gut_clusters <- c("Fibroblast", "Treg", "Tumor epithelial cell", "SPP1+ macrophage", "Endothelial cell", "Fibroblast",
                         "Tumor epithelial cell", "SPP1+ monocyte", "Plasma cell", "Pericyte", "Fibroblast", "Fibroblast", "Goblet cell", "Goblet cell",
                         "B cell", "Myofibroblast", "Plasma cell","SPP1+ monocyte", "Tuft cell", "B cell", "Stem cell", "Neutrophil", "Tumor epithelial cell")  
names(CIPR_colon_gut_clusters) <- levels(fov_stromal)
fov_stromal <- RenameIdents(fov_stromal, CIPR_colon_gut_clusters)
# Adding object metadata with
fov_stromal$CIPR_colon_gut_clusters <- factor(fov_stromal@active.ident)

Idents(fov_stromal) <- "st_harmony_clusters"
levels(fov_stromal)
CIPR_gut_clusters <- c("T reticular", "Paneth", "Stem cell", "SPP1+MMP12+ Macrophage", "Endothelial cell", "T reticular",
                       "Stem cell", "SPP1+CXCL8+CXCL1+ monocyte", "B cell", "Endothelial cell", "Paneth", "Endothelial cell", "Goblet cell", "TA epithelial cell",
                       "Plasma cell", "Pericyte", "Plasma cell","SPP1+ monocyte", "Colonocyte", "Myofibroblast", "BEST2 Goblet cell", "Neutrophil", "Plasma cell") 
names(CIPR_gut_clusters) <- levels(fov_stromal)
fov_stromal <- RenameIdents(fov_stromal, CIPR_gut_clusters )
# Adding object metadata with
fov_stromal$CIPR_gut_clusters <- factor(fov_stromal@active.ident)

dittoDimPlot(fov_stromal, var = "CIPR_colon_gut_clusters",
             reduction.use = "umap.harmony", size = 1,
             do.label = TRUE, labels.size = 4, labels.highlight = TRUE) +
  theme(legend.title = element_blank()) +
...
ImageDimPlot(fov_stromal, fov = "fov02", axes = TRUE, cols = "polychrome", size = 1, boundaries = "segmentation")
ImageDimPlot(fov_stromal, fov = "fov23", axes = TRUE, cols = "polychrome", size = 1, boundaries = "segmentation")

ImageDimPlot(fov_stromal, fov = "fov02", axes = TRUE, cols = "polychrome", size = 1, boundaries = "segmentation")
ImageFeaturePlot(fov_stromal, fov = "fov02", features = c('CD3E',"TNFRSF18", "FOXP3"), max.cutoff = "q95", boundaries = "segmentation")
ImageFeaturePlot(fov_stromal, fov = "fov23", features = c('CD3E',"TNFRSF18", "FOXP3"), max.cutoff = "q95", boundaries = "segmentation")

#ImageDimPlot(fov_stromal, fov = "fov23", cols = "polychrome", alpha = 0.6, molecules = c("TNFRSF18", "FOXP3"), mols.size = 0.3, nmols = 2000, border.color = "black", coord.fixed = FALSE, boundaries = "segmentation")

# Plot frequencies of SCSA labelled clusters per sample
dittoBarPlot(fov_stromal, var = "CIPR_colon_gut_clusters", group.by = "sample_ID",  scale = c("percent")) 
dittoBarPlot(fov_stromal, var = "CIPR_colon_gut_clusters", group.by = "CMS",  scale = c("percent")) 
dittoBarPlot(fov_stromal, var = "CIPR_colon_gut_clusters", group.by = "MMR_Status_Jen",  scale = c("percent")) 
...
DoHeatmap((subset(fov_stromal, downsample = 1000)), features = top10$gene, size = 3) + NoLegend() + theme(axis.text.y = element_text(size = 8))

dittoHeatmap(fov_stromal, top10$gene, 
             cluster_rows = FALSE, 
             annot.by = "CIPR_colon_gut_clusters", 
             fontsize_row = 8)

avgexp <- AverageExpression(subset(fov_stromal, features = top10$gene))
avgexp <- as.data.frame(avgexp$RNA)
write.csv(avgexp, "Average_expression_Top_expressed_genes_CIPR_stromal_clusters_RNA.csv")
#avgexp$gene <- rownames(avgexp)
#avgexp <- avgexp[,1:13]
...
colon_ref <- readRDS("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/colon_ref_insitutype.rds")
...
gut_ref <- readRDS("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/gut_ref_insitutype.rds")

# Customised gut refs data with non-stromal cells removed:
gut_ref2 <- read.csv("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/gut_ref4.csv", sep=",", row.names=1)
gut_ref3  <- as.matrix(gut_ref2)
str(gut_ref3)
saveRDS(gut_ref3, "gut_ref4_insitutype.rds")
gut_ref2 <- readRDS("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/gut_ref4_insitutype.rds")

# Customised colon+gut refs data:
gut_ref2 <- read.csv("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/gut_ref4.csv")
colon_ref2 <- read.csv("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/colon_ref.csv")
merged <- merge(colon_ref2, gut_ref2, by.x = "Gene", by.y = "Gene") # gave the best performance for the annotation
write.csv(x = merged, file = "colon_gut_ref2.csv", row.names = TRUE, quote = FALSE)

colon_gut_ref_stromal <- read.csv("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/colon_gut_ref_stromal.csv", sep=",", row.names=1)
colon_gut_ref_stromal2 <- as.matrix(colon_gut_ref_stromal)
str(colon_gut_ref_stromal2)
saveRDS(colon_gut_ref_stromal2, "./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/colon_gut_ref_stromal_insitutype.rds")
colon_gut_ref_stromal <- readRDS("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/colon_gut_ref_stromal_insitutype.rds")

# Customised colon+gut and epithelial+gut refs data:
ref1 <- read.csv("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/gut_ref_epithelial.csv")
...
merged <- merge(ref1, ref2, by.x = "Gene", by.y = "Gene") # gave the best performance for the annotation
merged2 <- merge(merged, ref3, by.x = "Gene", by.y = "Gene") # gave the best performance for the annotation
write.csv(x = merged2, file = "./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/colon_rectum_gut_ref.csv", row.names = TRUE, quote = FALSE)
ref4 <- read.csv("./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/colon_rectum_gut_ref.csv", sep=",", row.names=1)
ref5 <- as.matrix(ref4)
str(ref5)
saveRDS(ref5, "./RDS_files/7_Reference_datasets/CellProfileLibrary/Human/Insitutype/colon_rectum_gut_ref_insitutype.rds")
...
levels(fov_subset)
fov_subset <- subset(fov_subset, idents = c("Other"))

counts <- LayerData(fov_subset, assay = "RNA")
saveRDS(counts, "stromal_all_counts.rds")
...
saveRDS(neg, "stromal_negative_counts.rds")
...
sup <- insitutypeML(x = counts,
                    neg = negmean,
                    reference_profiles = colon_ref)   

Insitutype_colon <- sup$clust
write.csv(Insitutype_colon, "insituclusters_supervised_colon_ref.csv")

# Run with ileum ref
sup2 <- insitutypeML(x = counts,
                    neg = negmean,
                    reference_profiles = gut_ref)   

Insitutype_gut <- sup2$clust
write.csv(Insitutype_gut, "insituclusters_supervised_gut_ref.csv")

# Run with edited gut ref
sup3 <- insitutypeML(x = counts,
                     neg = negmean,
                     reference_profiles = gut_ref2)   

Insitutype_gut2 <- sup3$clust
write.csv(Insitutype_gut2, "insituclusters_supervised_gut_ref2.csv")

# Run with edited gut ref + colon ref
sup4 <- insitutypeML(x = counts,
                     neg = negmean,
                     reference_profiles = colon_gut_ref_stromal)   

Insitutype_colon_gut <- sup4$clust
write.csv(Insitutype_colon_gut, "insituclusters_supervised_colon_gut_ref_stromal.csv")

sup5 <- insitutypeML(x = counts,
                     neg = negmean,
                     reference_profiles = colon_rectum_gut_ref)   

Insitutype_colon_rectum_gut <- sup5$clust
write.csv(Insitutype_colon_rectum_gut, "insituclusters_supervised_colon_rectum_gut_ref_stromal.csv")
...
#fov_stromal$Insitutype_colon_rectum_gut <- NULL

DefaultAssay(fov_stromal) <- "RNA" 
Idents(fov_stromal) <- 'st_harmony_clusters'
levels(fov_stromal)
fov_stromal@meta.data <-cbind(fov_stromal@meta.data, Insitutype_colon_rectum_gut)

Idents(fov_stromal) <- "Insitutype_colon_rectum_gut"
levels(fov_stromal)

dittoDimPlot(fov_stromal, var = "Insitutype_colon_rectum_gut",
             reduction.use = "umap.harmony", size = 1,
             do.label = TRUE, labels.size = 2, labels.highlight = TRUE) +
  theme(legend.title = element_blank()) +
...
# Plot frequencies of SCSA labelled clusters per sample
dittoBarPlot(fov_stromal, var = "Insitutype_colon_gut", group.by = "sample_ID",  scale = c("percent")) 
dittoBarPlot(fov_stromal, var = "Insitutype_colon_gut", group.by = "CMS",  scale = c("percent")) 
dittoBarPlot(fov_stromal, var = "Insitutype_colon_gut", group.by = "MMR_Status_Jen",  scale = c("percent")) 

# Plot normalised protein expression per cluster
DefaultAssay(fov_stromal) <- "Protein"
Idents(fov_stromal) <- "Insitutype_colon_gut"
features <- c("MeanPanCK", "MaxPanCK","MeanCD45", "MaxCD45", "MeanCD3", "MaxCD3")

RidgePlot(fov_stromal, features = features, sort = TRUE, ncol = 2)

multi_dittoDimPlot(fov_stromal, vars = c("CIPR_colon_gut_clusters", "Insitutype_colon_gut_clusters"),
                   reduction.use = "umap.harmony", size = 1, ncol=2,
                   do.label = TRUE, labels.size = 2, labels.highlight = TRUE) +
  theme(legend.title = element_blank()) +
  ggtitle("Epithelial clusters on UMAP")
